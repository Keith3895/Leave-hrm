...
@Component({
  selector: 'bh-userdashboard',
  templateUrl: './userdashboard.template.html'
})

export class userdashboardComponent implements OnInit {
  @ViewChild('leaveForm') form;
  leaveRadiobtn: FormGroup;
  ...
  getTabChange(event) {
    if (event.index == '0' && event.tab['textLabel'] == 'Profile Details') {
      this.selectedTabIndex = 0;
      this.getUserDetails();
    } else if (event.index == '2' && event.tab['textLabel'] == 'Leave History') {
  ...
  resetFormValues(leaveForm) {
    console.log('from half', this.fromLeaveHalf);
    console.log('to half', this.toLeaveHalf);
    leaveForm.reset();
    this.showMessageBox = false;
    this.minDate = new Date();
  }
